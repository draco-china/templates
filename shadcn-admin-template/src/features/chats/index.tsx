			...
			<Main fixed>
				<section className="flex h-full gap-6">
					...
					<div className="flex w-full flex-col gap-2 sm:w-56 lg:w-72 2xl:w-80">
						...
						<ScrollArea className="-mx-3 h-full overflow-auto p-3">
												...
												Object.keys(currentMessage).map((key) => (
													<Fragment key={key}>
														{currentMessage[key].map((msg, index) => (
															<div
																className={cn(
																	"chat-box wrap-break-word max-w-72 px-3 py-2 shadow-lg",
																	msg.sender === "You"
																		? "self-end rounded-[16px_16px_0_16px] bg-primary/90 text-primary-foreground/75"
																		: "self-start rounded-[16px_16px_16px_0] bg-muted"
																)}
																key={`${msg.sender}-${msg.timestamp}-${
																	// biome-ignore lint/suspicious/noArrayIndexKey: timestamps can repeat, so we need the index
																	index
																}`}
															>
																{msg.message}{" "}
