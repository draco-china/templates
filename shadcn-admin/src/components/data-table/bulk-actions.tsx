...
export function DataTableBulkActions<TData>({
  ...
  useEffect(() => {
    if (selectedCount > 0) {
      const message = t(
        'bulkActions.selectedMessage',
        '{{count}} {{entityName}} selected. Bulk actions toolbar is available.',
        {
          count: selectedCount,
          entityName: entityName + (selectedCount > 1 ? 's' : ''),
        },
      );
  ...
  return (
    <>
      ...
      <div
        ...
        role="toolbar"
        aria-label={t(
          'bulkActions.toolbarLabel',
          'Bulk actions for {{count}} selected {{entityName}}',
          {
            count: selectedCount,
            entityName: entityName + (selectedCount > 1 ? 's' : ''),
          },
        ...
        >
          <Tooltip>
            <TooltipTrigger asChild>
              <Button
                ...
                aria-label={t('bulkActions.clearSelection', 'Clear selection')}
                title={t(
                  'bulkActions.clearSelectionShortcut',
                  'Clear selection (Escape)',
                )}
              >
                <X />
                <span className="sr-only">
                  {t('bulkActions.clearSelection', 'Clear selection')}
                </span>
              </Button>
            </TooltipTrigger>
            <TooltipContent>
          ...
          >
            <Badge
              ...
              aria-label={t('bulkActions.countSelected', '{{count}} selected', {
                count: selectedCount,
              })}
            >
              {selectedCount}
            ...
            {t('bulkActions.selected', 'selected')}
          </div>
